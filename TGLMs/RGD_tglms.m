...
% FOR THE LOCAL CONTRACTION OF TENSOR Generalized Linear Models
function [errl,time] = RGD_tglms( A_mat,y,X0, U0,X, U, p1, p2, p3, r1,r2,r3, iter_max,model,normal)
...
for iter = 1:iter_max    
    ...
    if model == "logistic"
        haty = 1./(1+exp(-haty));
        Z = reshape( 4.84 * (haty - y)' * A_mat/m, [p1, p2, p3] );  
    elseif model == "probit"
        haty = normcdf(haty);
        Z = reshape( 3.545 * (haty - y)' * A_mat/m, [p1, p2, p3] ); 
    elseif model == "logiviasim"
        Z = reshape( (haty - 2*y)' * A_mat/m, [p1, p2, p3] );
    else  %% model == 'poisson'
        haty = exp(haty);
        Z = reshape( 0.607 * (haty - y)' * A_mat/m, [p1, p2, p3] );  
    end
    Z = tensor(Z);
    ...
    alpha_t = 1; %% already rescale gradient in Z above
