...
% FOR THE LOCAL CONTRACTION OF FGD for One-Bit Matrix Sensing
function errl = FGD_1bms(A_mat,y,X0,X,eta,n,r,gradient,reb,iter_max)
Xt = X0;
[U,S,V]=svd(Xt);
...
for iter = 1:iter_max    
    ...
    if gradient == "1bms"
        haty = sign(haty);
        H = reshape((haty - y)' * A_mat/m, [n,n]);
    else
        H = reshape((haty - sqrt(pi/2)*y)' * A_mat/m, [n,n]);
    end
    % FGD
    Ut = Ut - eta *(H* Vt);
    Vt = Vt - eta *(H'* Ut);
