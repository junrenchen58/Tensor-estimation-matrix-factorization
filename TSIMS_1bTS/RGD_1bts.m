...
% FOR THE LOCAL CONTRACTION OF TENSOR SINGLE INDEX MODELS
function [errl,time] = RGD_1bts( A_mat,y,X0, U0,X, U, p1, p2, p3, r1,r2,r3, iter_max)
...
for iter = 1:iter_max    
    haty = A_mat * Xt(:);
    haty = sign(haty);
    Z = reshape( sqrt(pi/2)*(haty - y)' * A_mat/m, [p1, p2, p3] ); %% gradient changes to the 1bCS one
    Z = tensor(Z);
    ...
    Xt = tensor(Xt);
    Xt = Xt/norm(Xt); %% Normalization needed for SIMs
